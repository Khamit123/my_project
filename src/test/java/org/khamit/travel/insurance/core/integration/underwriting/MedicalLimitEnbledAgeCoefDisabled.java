...
public class MedicalLimitEnbledAgeCoefDisabled {
    @Autowired
    TravelPremiumUnderwriting travelPremiumUnderwriting;
    @Test
    public void MedicalLimitEnbledAgeCoefDisabledWorking(){
        ...
        assertTrue(BigDecimal.valueOf(900).compareTo(info.getRiskPremuimInfo().getFirst().getRiskAmount())==0);
        assertTrue(BigDecimal.valueOf(10).compareTo(info.getRiskPremuimInfo().getLast().getRiskAmount())==0);
        assertTrue(BigDecimal.valueOf(910).compareTo(info.getTotalPremium())==0);
