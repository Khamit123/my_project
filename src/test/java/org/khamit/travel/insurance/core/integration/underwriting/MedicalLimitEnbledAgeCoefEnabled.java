...
public class MedicalLimitEnbledAgeCoefEnabled {
    @Autowired
    TravelPremiumUnderwriting travelPremiumUnderwriting;
    @Test
    public void MedicalLimitEnbledAgeCoefEnbleddWorking(){
        ...
        assertTrue(BigDecimal.valueOf(1080).compareTo(info.getRiskPremuimInfo().getFirst().getRiskAmount())==0);
        assertTrue(BigDecimal.valueOf(10).compareTo(info.getRiskPremuimInfo().getLast().getRiskAmount())==0);
        assertTrue(BigDecimal.valueOf(1090).compareTo(info.getTotalPremium())==0);
